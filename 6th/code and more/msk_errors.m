...
bi=sign(sum(xi.*gmi));
bq=sign(sum(xq.*gmq));
% χωρίς προκωδικοποίηση: απο-σχολιάστε τις επόμενες 2 γρ.
%xr(k)=bi_1*bq;
%xr(k+1)=bi*bq;
% με προκωδικοποίηση: απο-σχολιάστε τις επόμενες 2 γραμμές
xr(k)=xr_1*bi_1*bq;
xr(k+1)=xr(k)*bi*bq;
xr_1=xr(k+1);
end
xr=xr';
%err=not(x==xr);
err=not(y==xr); % απο-σχολιάστε, με προκωδικοποίηση
errors=sum(err);
end
